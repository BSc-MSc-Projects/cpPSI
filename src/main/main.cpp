...
int main (int argc, char *argv[])
{
	...
	/* The poly modulus degree, one of the critical parameters to choose. The higher it is, the slower the operations will be but more complicated operations will be supported 
	 * Recommended values are: 1024, 2048, 4096, 8192, ..., 32768 */
	size_t poly_mod_degree = 8192;
	params.set_poly_modulus_degree(poly_mod_degree);

	/* sets up the coeff modulus, using the general good choice given the value of the poly mod degree */
	...
	vector<Ciphertext> send_encr_result = homomorphic_computation(recv_encr_data, params);
